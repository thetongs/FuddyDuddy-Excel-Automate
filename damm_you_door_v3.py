...
def excel_door_open(s_filepath, d_filepath):
    ...
    for sheet_name in book.sheetnames[2:]:
        ...
        #            
        if(sheet_name.strip().replace(".", '').isdigit()):
            book[sheet_name]["F3"].value = float(sheet_name.strip())
            book[sheet_name]['F3'].number_format  = numbers.FORMAT_NUMBER
        else:
            book[sheet_name]["F3"].value = sheet_name
            book[sheet_name]['F3'].number_format  = numbers.FORMAT_GENERAL
